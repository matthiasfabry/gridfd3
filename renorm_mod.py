...
folder = '9_Sgr/resubscalesnr3/fd3'
# %%
lfs = [0.62, 0.38]
thresh = 0.003
smooth = 0.03
...
# %%
np.savetxt(folder + '/{}primary.txt'.format(name),
           np.array([x[0], x[1], errorp * np.ones(len(x[1]))]).T)
np.savetxt(folder + '/{}secondary.txt'.format(name),
           np.array([x[0], x[2], errors * np.ones(len(x[2]))]).T)
